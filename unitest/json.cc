...
DEF_test(json) {
    ...
    DEF_case(object) {
        ...
        json::Json u = json::parse(v.str());
        EXPECT(u.is_object());
        EXPECT_EQ(u.size(), 3);
        EXPECT_EQ(u["name"].as_string(), "vin");
        EXPECT_EQ(u["age"].as_int(), 29);

        json::Json x = json::parse(v.pretty());
        EXPECT(x.is_object());
        EXPECT_EQ(x.size(), 3);

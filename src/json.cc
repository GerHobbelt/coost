...
struct Cache {
    ...
    Cache() : _stack(), _ustack(32 - R), _fs(256) {}
...
inline _H* make_object(C& c) { return new(c.alloc()) _H(Json::_obj_t()); }
inline _H* make_array(C& c)  { return new(c.alloc()) _H(Json::_arr_t()); }
...
digit_end:
    ...
    }
    {
        ...
        if (m < 0) goto to_int;
        if (m > 0) goto to_dbl;
        v = make_int(_c, *b != '-' ? MAX_UINT64 : MIN_INT64);
        return p - 1;
    }
    ...
    if (is_obj) {
        ...
        if (n > 0) {
            ...
            for (auto& x : v) {
                ...
                a.push_back(x[1]._h); ((Json&)x[1])._h = 0;
            }
        }
